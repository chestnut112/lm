...
bool Address::GetInterfaceAddresses(
  ...
  if (getifaddrs(&results) != 0) {
    ARVIN_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                 " err="
  ...
  try {
    for (next = results; next; next = next->ifa_next) {
      Address::ptr addr;
      ////无符号整型,~0u = 4294967295   <=> 11111111 11111111 11111111 11111111
    ...
    ARVIN_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
    freeifaddrs(results);
    return false;
  }
  // 释放results
...
IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
  ...
  if (error) {
    ARVIN_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address << ", " << port
...
IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
  ...
  if (result <= 0) {
    ARVIN_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
...
IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port) {
  ...
  if (result <= 0) {
    ARVIN_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                              << port << ") rt=" << result << " errno=" << errno
                              << " errstr=" << strerror(errno);
    return nullptr;
  }
