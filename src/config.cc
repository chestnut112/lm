...
void Config::LoadFromConfDir(const std::string &path, bool force) {
  ...
  for (auto &i : files) {
    ...
    try {
      ...
      LoadFromYaml(root);
      ARVIN_LOG_INFO(g_logger) << "LoadConfFile file=" << i << " ok";
    } catch (...) {
      ARVIN_LOG_ERROR(g_logger) << "LoadConfFile file=" << i << " failed";
    }
  }
}
